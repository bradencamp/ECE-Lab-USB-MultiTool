...
class Connection:
            ...
            if len(buff) == 0: #timeout 
                ...
                if(buff[0:9] == bytes("\0STMAWG23", "ascii")): #ack packet
                    ...
                    try:
                        ...
                        #print(buff)
                        if(buffpos < self.lastPos): #Ch1):
                           self.itteration += 1
                        for i in range(1,9):
                            self.buffPosQueue.append(buffpos+(i-1)+self.itteration*BUFFERSIZE)
                            self.oscCh1Queue.append(int(tempInput[i]))
                            self.oscCh2Queue.append(int(tempInput[1]))
                        self.lastPos = buffpos'''#Ch1 = buffpos
                        #self.lastPosCh2 = buffpos
    ...
    def returnData(self): #return data. Change this to a full buffer
        return self.dataIn  
    def returnBuffPositions(self):
        return asarray(self.buffPosQueue)      
    def returnLogicPositions(self):
        return asarray(self.LogicPosQueue)

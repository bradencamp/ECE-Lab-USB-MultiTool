...
class Input(QLineEdit):
    ...
    def keyPressEvent(self, event):
        ...
        match event.key():
            case 16777235:
                self.setVal(self.value + 1*self.prefixes[self.prefix])
            case 16777237:
                self.setVal(self.value - 1*self.prefixes[self.prefix])
            case _:
    ...
    def wheelEvent(self, event):
        ...
        event (QKeyEvent): The key event that was triggered.
        """
        self.setVal(self.value + 0.1*self.value*event.angleDelta().y() / 120)
        event.accept()
    ...
    def setVal(self, val, runCallback = True):
        ...
        self.prefix = prefix
        self.setText(f"{val:.{self.numDec}f} {prefix}{self.def_unit}")
        if runCallback:
            self.callback_update()
        
    def __init__(self, callback_update, range, init_val,precision, def_unit, prefixes = [], *args, **kwargs):

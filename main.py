    ...
    def update_plot(self):
        ...
        if not self.runStopButton.isChecked():
            ...
            if self.oscilloCheck.isChecked():   
                self.conn.sendWave(1, freq = self.awgCh1Config["freq"], wave_type = self.waves[self.awgCh1Config["wave"]], amplitude = self.awgCh1Config["amp"], offset = self.awgCh1Config["off"], arbitrary_waveform = "lol", duty = 50, phase = self.awgCh1Config["phase"] )
                print("Sent wave")
                if self.oscCh1EN.isChecked():
                   self.graphOscCh1()
